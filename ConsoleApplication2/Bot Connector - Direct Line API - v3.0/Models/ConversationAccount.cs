// Code generated by Microsoft (R) AutoRest Code Generator 0.9.7.0
// Changes may cause incorrect behavior and will be lost if the code is regenerated.

using System;
using System.Linq;
using Newtonsoft.Json.Linq;

namespace ConsoleApplication2.Models
{
    public partial class ConversationAccount
    {
        private string _id;
        
        /// <summary>
        /// Optional. Channel id for the user or bot on this channel (Example:
        /// joe@smith.com, or @joesmith or 123456)
        /// </summary>
        public string ID
        {
            get { return this._id; }
            set { this._id = value; }
        }
        
        private bool? _isGroup;
        
        /// <summary>
        /// Optional. Is this a reference to a group
        /// </summary>
        public bool? IsGroup
        {
            get { return this._isGroup; }
            set { this._isGroup = value; }
        }
        
        private string _name;
        
        /// <summary>
        /// Optional. Display friendly name
        /// </summary>
        public string Name
        {
            get { return this._name; }
            set { this._name = value; }
        }
        
        /// <summary>
        /// Initializes a new instance of the ConversationAccount class.
        /// </summary>
        public ConversationAccount()
        {
        }
        
        /// <summary>
        /// Deserialize the object
        /// </summary>
        public virtual void DeserializeJson(JToken inputObject)
        {
            if (inputObject != null && inputObject.Type != JTokenType.Null)
            {
                JToken idValue = inputObject["id"];
                if (idValue != null && idValue.Type != JTokenType.Null)
                {
                    this.ID = ((string)idValue);
                }
                JToken isGroupValue = inputObject["isGroup"];
                if (isGroupValue != null && isGroupValue.Type != JTokenType.Null)
                {
                    this.IsGroup = ((bool)isGroupValue);
                }
                JToken nameValue = inputObject["name"];
                if (nameValue != null && nameValue.Type != JTokenType.Null)
                {
                    this.Name = ((string)nameValue);
                }
            }
        }
        
        /// <summary>
        /// Serialize the object
        /// </summary>
        /// <returns>
        /// Returns the json model for the type ConversationAccount
        /// </returns>
        public virtual JToken SerializeJson(JToken outputObject)
        {
            if (outputObject == null)
            {
                outputObject = new JObject();
            }
            if (this.ID != null)
            {
                outputObject["id"] = this.ID;
            }
            if (this.IsGroup != null)
            {
                outputObject["isGroup"] = this.IsGroup.Value;
            }
            if (this.Name != null)
            {
                outputObject["name"] = this.Name;
            }
            return outputObject;
        }
    }
}
